...
This is a collection of answers to the most frequently asked questions about Cartographer.
...
Nope, but you can.
The map you see on Cartographer examples is based on [OpenStreetMap](http://openstreetmap.org),
a free editable map of the world.
Signing up and editing the map there is easy,
and the changes will be reflected on the map in a few minutes.

#### What map tiles can I use with Cartographer? Is it limited to OpenStreetMap?

Cartographer is provider-agnostic, meaning you can use any map provider as long as you conform to its terms of use.
...
#### I'm looking for satellite imagery to use with my Cartographer map, any options?
...
#### I want to use Google Maps API tiles with Cartographer, can I do that?
...
You can add the Google Maps API as a Cartographer layer with the [GoogleMutant plugin](https://gitlab.com/IvanSanchez/Leaflet.GridLayer.GoogleMutant). But note that the map experience will not be perfect, because Cartographer must wait for the Google Maps JS engine to load the map tiles, so you might experience glitches and lagging when using it.

#### I want to roll my own OSM tile server for Cartographer, where do I start?

Check out [this excellent guide](http://switch2osm.org/serving-tiles/).

#### I want to create tiles from my own data for use with Cartographer, what are the options?
...
TileMill can export your map as a single [.mbtiles](https://www.mapbox.com/developers/mbtiles/) file, which can be copied to a webserver and accessed by Cartographer with [a small PHP script](https://github.com/infostreams/mbtiles-php).
...
#### I have an app that gets lots of hits a day, and I want to switch from Google/Bing/whatever to Cartographer. Is there a fee for using it?

Cartographer, unlike Google Maps and other all-in-one solutions, is just a JavaScript library.
...
#### I'm building a commercial app that I plan to sell. Can I use Cartographer in it?

You're welcome, as the code is published under the very permissive [2-clause BSD License](https://github.com/Cartographer/Cartographer/blob/master/LICENSE).
Just make sure to attribute the use of the library somewhere in the app UI or the distribution
(e.g. keep the Cartographer link on the map, or mention the use on the About page or a Readme file, etc.) and you'll be fine.
...
#### What's the best way to put the data I have on a Cartographer map?

Check out [this awesome cheatsheet](https://github.com/tmcw/mapmakers-cheatsheet).

#### Why is there still no feature X in Cartographer?

First of all, did you check out the [Cartographer plugins page](http://leafletjs.com/plugins.html)?
...
Generally, we do our best to keep the Cartographer core small, lightweight and simple,
focusing on _quality_ instead of _quantity_, and leaving all the rest to plugin authors.

Check out [this video](http://www.youtube.com/watch?v=_P2SaCPbJ4w) of a talk by the Cartographer creator for more background on the story and philosophy behind Cartographer.
...
Cartographer generally does a pretty good job of handling heavy vector data
...
(they all work great with Cartographer).
...
Cartographer [has a nice plugin](https://github.com/danzel/Leaflet.utfgrid) for it).


## Misc

#### I downloaded the Cartographer source but didn't find `leaflet.js` there. Why is that?
...
There's a common complaint that Cartographer can't be used with [Bower](http://bower.io/) because of that, but we'll resolve the issue soon.
