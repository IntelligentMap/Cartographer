describe("Map", function () {
	...
	describe("#remove", function () {
		it("fires an unload event if loaded", function () {
			var container = document.createElement('div'),
			    map = new Cartographer.Map(container).setView([0, 0], 0),
			    spy = sinon.spy();
			map.on('unload', spy);
			map.remove();
		...
		it("fires no unload event if not loaded", function () {
			var container = document.createElement('div'),
			    map = new Cartographer.Map(container),
			    spy = sinon.spy();
			map.on('unload', spy);
			map.remove();
		...
		describe("corner case checking", function () {
			...
			it("throws an exception if a container is not found", function () {
				expect(function () {
					Cartographer.map('nonexistentdivelement');
				}).to.throwException(function (e) {
		...
		it("unbinds events", function () {
			var container = document.createElement('div'),
			    map = new Cartographer.Map(container).setView([0, 0], 1),
			    spy = sinon.spy();
		...
		it("throws error if container is reused by other instance", function () {
			...
			map.remove();
			map2 = Cartographer.map(container);
	...
	describe('#getCenter', function () {
		...
		it('returns a precise center when zoomed in after being set (#426)', function () {
			var center = Cartographer.latLng(10, 10);
		...
		it('returns correct center after invalidateSize (#1919)', function () {
			map.setView(Cartographer.latLng(10, 10), 1);
			map.invalidateSize();
			expect(map.getCenter()).not.to.eql(Cartographer.latLng(10, 10));
		});
	});
	...
	describe("#setView", function () {
		...
		it("defaults to zoom passed as map option", function () {
			map = Cartographer.map(document.createElement('div'), {zoom: 13});
			expect(map.setView([51.605, -0.11])).to.be(map);
			expect(map.getZoom()).to.be(13);
		});

		it("passes duration option to panBy", function () {
			map = Cartographer.map(document.createElement('div'), {zoom: 13, center: [0, 0]});
			map.panBy = sinon.spy();
	...
	describe("#getBoundsZoom", function () {
		...
		it.skipInPhantom("returns multiples of zoomSnap when zoomSnap > 0 on any3d browsers", function () {
			...
			container.style.height = height;
			document.body.appendChild(container);
			// Cartographer.Browser.any3d = true;	// Cartographer.Browser is frozen since ES6ication
		...
		it("getBoundsZoom does not return Infinity when projected SE - NW has negative components", function () {
			...
			document.body.appendChild(container);
			var bounds = Cartographer.latLngBounds(Cartographer.latLng([62.18475569507688, 6.926335173954951]), Cartographer.latLng([62.140483526511694, 6.923933370740089]));
			...
			var projectedSE = Cartographer.point(7800503.059925064, 6440062.353052008);
			var projectedNW = Cartographer.point(7801987.203481699, 6425186.447901004);
			...
			var padding = Cartographer.point(-50, -50);
			map.setZoom(16);
	...
	describe('#setMaxBounds', function () {
		...
		it("aligns pixel-wise map view center with maxBounds center if it cannot move view bounds inside maxBounds (#1908)", function () {
			...
			// maxBounds
			var bounds = Cartographer.latLngBounds([51.5, -0.05], [51.55, 0.05]);
			map.setMaxBounds(bounds, {animate: false});
			// set view outside
			map.setView(Cartographer.latLng([53.0, 0.15]), 12, {animate: false});
		...
		it("moves map view within maxBounds by changing one coordinate", function () {
			...
			// maxBounds
			var bounds = Cartographer.latLngBounds([51, -0.2], [52, 0.2]);
			map.setMaxBounds(bounds, {animate: false});
			...
			map.setView(Cartographer.latLng(initCenter), 16, {animate: false});
		...
		it("remove listeners when called without arguments", function () {
			Cartographer.tileLayer('http://tilecache.openstreetmap.fr/hot/{z}/{x}/{y}.png', {minZoom: 0, maxZoom: 20}).addTo(map);
			container.style.width = container.style.height = "500px";
			var bounds = Cartographer.latLngBounds([51.5, -0.05], [51.55, 0.05]);
			map.setMaxBounds(bounds, {animate: false});
			map.setMaxBounds();
			// set view outside
			var center = Cartographer.latLng([0, 0]);
	...
	describe("#getMinZoom and #getMaxZoom", function () {
		...
		it("minZoom and maxZoom options overrides any minZoom and maxZoom set on layers", function () {
			...
			var map = Cartographer.map(document.createElement('div'), {minZoom: 2, maxZoom: 20});

			Cartographer.tileLayer("{z}{x}{y}", {minZoom: 4, maxZoom: 10}).addTo(map);
			Cartographer.tileLayer("{z}{x}{y}", {minZoom: 6, maxZoom: 17}).addTo(map);
			Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 22}).addTo(map);
		...
		it("layer minZoom overrides map zoom if map has no minZoom set and layer minZoom is bigger than map zoom", function () {
			var map = Cartographer.map(document.createElement("div"), {zoom: 10});
			Cartographer.tileLayer("{z}{x}{y}", {minZoom: 15}).addTo(map);
		...
		it("layer maxZoom overrides map zoom if map has no maxZoom set and layer maxZoom is smaller than map zoom", function () {
			var map = Cartographer.map(document.createElement("div"), {zoom: 20});
			Cartographer.tileLayer("{z}{x}{y}", {maxZoom: 15}).addTo(map);
	...
	describe("#addLayer", function () {
		...
		it("throws if adding something which is not a layer", function () {
			var control = Cartographer.control.layers();
			expect(function () {
		...
		describe("When the first layer is added to a map", function () {
			it("fires a zoomlevelschange event", function () {
				...
				expect(spy.called).not.to.be.ok();
				Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 10}).addTo(map);
				expect(spy.called).to.be.ok();
			});
		});
		...
		describe("when a new layer with greater zoomlevel coverage than the current layer is added to a map", function () {
			it("fires a zoomlevelschange event", function () {
				...
				map.on("zoomlevelschange", spy);
				expect(spy.called).not.to.be.ok();
				Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 15}).addTo(map);
				expect(spy.called).to.be.ok();
			});
		});
		...
		describe("when a new layer with the same or lower zoomlevel coverage as the current layer is added to a map", function () {
			it("fires no zoomlevelschange event", function () {
				...
				Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 10}).addTo(map);
				expect(spy.called).not.to.be.ok();
				Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 5}).addTo(map);
				expect(spy.called).not.to.be.ok();
			});
		});
	});
	...
	describe("#removeLayer", function () {
		...
		it("supports adding and removing a tile layer without initializing the map", function () {
			var layer = Cartographer.tileLayer("{z}{x}{y}");
			map.addLayer(layer);
			map.removeLayer(layer);
		});
		...
		it("supports adding and removing a tile layer without initializing the map", function () {
			map.setView([0, 0], 18);
			var layer = Cartographer.gridLayer();
			map.addLayer(layer);
			map.removeLayer(layer);
		});
		...
		describe("when the last tile layer on a map is removed", function () {
			it("fires a zoomlevelschange event", function () {
				map.whenReady(function () {
					var spy = sinon.spy();
					var tl = Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 10}).addTo(map);
		...
		describe("when a tile layer is removed from a map and it had greater zoom level coverage than the remainding layer", function () {
			it("fires a zoomlevelschange event", function () {
				map.whenReady(function () {
					var spy = sinon.spy(),
					    tl = Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 10}).addTo(map),
					    t2 = Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 15}).addTo(map);
		...
		describe("when a tile layer is removed from a map it and it had lesser or the sa,e zoom level coverage as the remainding layer(s)", function () {
			it("fires no zoomlevelschange event", function () {
				map.whenReady(function () {
					var tl = Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 10}).addTo(map),
					    t2 = Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 10}).addTo(map),
					    t3 = Cartographer.tileLayer("{z}{x}{y}", {minZoom: 0, maxZoom: 5}).addTo(map);
	...
	describe("#eachLayer", function () {
		...
		it("calls the provided function for each layer", function () {
			var t1 = Cartographer.tileLayer("{z}{x}{y}").addTo(map),
			    t2 = Cartographer.tileLayer("{z}{x}{y}").addTo(map),
			    spy = sinon.spy();
	...
	describe('#flyTo', function () {
		...
		it('move to requested center and zoom, and call zoomend once', function (done) {
			...
			var spy = sinon.spy(),
			    newCenter = new Cartographer.LatLng(10, 11),
			    newZoom = 12;
		...
		it('flyTo start latlng == end latlng', function (done) {
			...
			var dc = new Cartographer.LatLng(38.91, -77.04);
			map.setView(dc, 14);
	...
	describe('#zoomIn and #zoomOut', function () {
		var center = Cartographer.latLng(22, 33);
		beforeEach(function () {
		...
		it.skipInNonPhantom('zoomIn ignores the zoomDelta option on non-any3d browsers', function (done) {
			Cartographer.Browser.any3d = false;
		...
		it.skipInPhantom('zoomIn respects the zoomDelta option on any3d browsers', function (done) {
			Cartographer.Browser.any3d = true;
			map.options.zoomSnap = 0.25;
			map.options.zoomDelta = 0.25;
			map.setView(center, 10);
			map.once('zoomend', function () {
	...
	describe('#_getBoundsCenterZoom', function () {
		var center = Cartographer.latLng(50.5, 30.51);
	...
	describe('#fitBounds', function () {
		var center = Cartographer.latLng(50.5, 30.51),
		    bounds = Cartographer.latLngBounds(Cartographer.latLng(1, 102), Cartographer.latLng(11, 122)),
		...
		it('Fits to small bounds from small zoom', function (done) {
			...
			bounds = Cartographer.latLngBounds([57.73, 11.93], [57.75, 11.95]);
			boundsCenter = bounds.getCenter();
			map.setZoom(0);
		});
		...
		it('Fits to large bounds from large zoom', function (done) {
			...
			bounds = Cartographer.latLngBounds([90, -180], [-90, 180]);
			boundsCenter = bounds.getCenter();
			map.setZoom(22);
		});
	});
	...
	describe('#fitBounds after layers set', function () {
		...
		it('Snaps to a number after adding tile layer', function (done) {
			Cartographer.Browser.any3d = true;
			map.addLayer(Cartographer.tileLayer('file:///dev/null'));
		...
		it('Snaps to a number after adding marker', function (done) {
			Cartographer.Browser.any3d = true;
			map.addLayer(Cartographer.marker(center));
			expect(map.getZoom()).to.be(undefined);
			map.fitBounds(bounds);
			expect(map.getZoom()).to.be(2);
			done();
	...
	describe("#panInside", function () {
		...
		it("pans the map when padding is provided and the target is within the border area", function () {
			...
			expect(distanceMoved.equals(Cartographer.point([-10, -20]))).to.eql(true);

			tlPix = map.getPixelBounds().min;
			p = [map.getPixelBounds().max.x - 10, map.getPixelBounds().min.y];	// Top-right
			map.panInside(map.unproject(p), {padding: padding, animate: false});
			distanceMoved = map.getPixelBounds().min.subtract(tlPix);
			expect(distanceMoved.equals(Cartographer.point([30, -20]))).to.eql(true);

			tlPix = map.getPixelBounds().min;
			p = [map.getPixelBounds().min.x + 35, map.getPixelBounds().max.y];	// Bottom-left
			map.panInside(map.unproject(p), {padding: padding, animate: false});
			distanceMoved = map.getPixelBounds().min.subtract(tlPix);
			expect(distanceMoved.equals(Cartographer.point([-5, 20]))).to.eql(true);

			tlPix = map.getPixelBounds().min;
			p = [map.getPixelBounds().max.x - 15, map.getPixelBounds().max.y]; // Bottom-right
			map.panInside(map.unproject(p), {padding: padding, animate: false});
			distanceMoved = map.getPixelBounds().min.subtract(tlPix);
			expect(distanceMoved.equals(Cartographer.point([25, 20]))).to.eql(true);
		});
		...
		it("supports different padding values for each border", function () {
			...
			map.panInside(map.unproject(Cartographer.point(br.x - 20, br.y)), opts);
		...
		it("pans only on the Y axis when the target's X coord is within bounds but the Y is not", function () {
			var p = Cartographer.latLng(tl.lat + 5, tl.lng);
			map.panInside(p, {animate: false});
			expect(map.getBounds().contains(p)).to.be(true);
		...
		it("pans only on the X axis when the target's Y coord is within bounds but the X is not", function () {
			var p = Cartographer.latLng(tl.lat, tl.lng - 5);
	...
	describe('#DOM events', function () {
		...
		it("DOM events propagate from marker to map", function () {
			var spy = sinon.spy();
			map.on("mousemove", spy);
			var layer = new Cartographer.Marker([1, 2]).addTo(map);
		...
		it("DOM events fired on marker can be cancelled before being caught by the map", function () {
			...
			var layer = new Cartographer.Marker([1, 2]).addTo(map);
			layer.on("mousemove", Cartographer.DomEvent.stopPropagation).on("mousemove", layerSpy);
			happen.mousemove(layer._icon);
		...
		it("mouseout is forwarded if fired on the original target", function () {
			var mapSpy = sinon.spy(),
			    ...
			    otherSpy = sinon.spy();
			var layer = new Cartographer.Polygon([[1, 2], [3, 4], [5, 6]]).addTo(map);
			var other = new Cartographer.Polygon([[10, 20], [30, 40], [50, 60]]).addTo(map);
		...
		it("mouseout is forwarded when using a DivIcon", function () {
			var icon = Cartographer.divIcon({
				html: "<p>this is text in a child element</p>",
				iconSize: [100, 100]
			});
			var mapSpy = sinon.spy(),
			    layerSpy = sinon.spy(),
			    layer = Cartographer.marker([1, 2], {icon: icon}).addTo(map);
			map.on("mouseout", mapSpy);
			layer.on("mouseout", layerSpy);
