describe('LatLng', function () {
	describe('constructor', function () {
		it("sets lat and lng", function () {
			var a = new Cartographer.LatLng(25, 74);
			expect(a.lat).to.eql(25);
			expect(a.lng).to.eql(74);

			var b = new Cartographer.LatLng(-25, -74);
		...
		it('throws an error if invalid lat or lng', function () {
			expect(function () {
				var a = new Cartographer.LatLng(NaN, NaN);
			}).to.throwError();
		});
		...
		it('sets altitude', function () {
			var a = new Cartographer.LatLng(25, 74, 50);
			expect(a.alt).to.eql(50);

			var b = new Cartographer.LatLng(-25, -74, -50);
	...
	describe('#equals', function () {
		it("returns true if compared objects are equal within a certain margin", function () {
			var a = new Cartographer.LatLng(10, 20);
			var b = new Cartographer.LatLng(10 + 1.0E-10, 20 - 1.0E-10);
			expect(a.equals(b)).to.eql(true);
		});

		it("returns false if compared objects are not equal within a certain margin", function () {
			var a = new Cartographer.LatLng(10, 20);
			var b = new Cartographer.LatLng(10, 23.3);
	...
	describe('#toString', function () {
		it('formats a string', function () {
			var a = new Cartographer.LatLng(10.333333333, 20.2222222);
	...
	describe('#distanceTo', function () {
		it('calculates distance in meters', function () {
			var a = new Cartographer.LatLng(50.5, 30.5);
			var b = new Cartographer.LatLng(50, 1);
		...
		it('does not return NaN if input points are equal', function () {
			var a = new Cartographer.LatLng(50.5, 30.5);
			var b = new Cartographer.LatLng(50.5, 30.5);

			expect(a.distanceTo(b)).to.eql(0);
		});
	});

	describe('Cartographer.latLng factory', function () {
		it('returns LatLng instance as is', function () {
			var a = new Cartographer.LatLng(50, 30);

			expect(Cartographer.latLng(a)).to.eql(a);
		});

		it('accepts an array of coordinates', function () {
			expect(Cartographer.latLng([])).to.eql(null);
			expect(Cartographer.latLng([50])).to.eql(null);
			expect(Cartographer.latLng([50, 30])).to.eql(new Cartographer.LatLng(50, 30));
			expect(Cartographer.latLng([50, 30, 100])).to.eql(new Cartographer.LatLng(50, 30, 100));
		});

		it('passes null or undefined as is', function () {
			expect(Cartographer.latLng(undefined)).to.eql(undefined);
			expect(Cartographer.latLng(null)).to.eql(null);
		});
		...
		it('accepts an object with lat/lng', function () {
			expect(Cartographer.latLng({lat: 50, lng: 30})).to.eql(new Cartographer.LatLng(50, 30));
		});

		it('accepts an object with lat/lon', function () {
			expect(Cartographer.latLng({lat: 50, lon: 30})).to.eql(new Cartographer.LatLng(50, 30));
		});

		it('returns null if lng not specified', function () {
			expect(Cartographer.latLng(50)).to.be(null);
		});
		...
		it('accepts an object with alt', function () {
			expect(Cartographer.latLng({lat: 50, lng: 30, alt: 100})).to.eql(new Cartographer.LatLng(50, 30, 100));
			expect(Cartographer.latLng({lat: 50, lon: 30, alt: 100})).to.eql(new Cartographer.LatLng(50, 30, 100));
		});
	});
	...
	describe('#clone', function () {
		...
		it('should clone attributes', function () {
			var a = new Cartographer.LatLng(50.5, 30.5, 100);
			var b = a.clone();
